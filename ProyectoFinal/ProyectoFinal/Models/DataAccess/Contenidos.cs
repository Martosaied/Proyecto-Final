...
namespace ProyectoFinal.Models.DataAccess
{
    ...
    {
        public static int Pagina { get; set; } = 1;
        ...
        public static void UpdatePopularidad(int id, int pop)
        {
            pop++;
        ...
        public static void UpdateDescargas(int id, int des)
        {
            des++;
        ...
        public static string GetGeneric(string Command)
        {
            string Get = "SELECT contenido.IDescargas, contenido.IPopularidad, contenido.Fechadesubida, usuarios.Nombre AS Usuario, contenido.IdContenido, contenido.Ruta, contenido.Nombre, "
        ...
        public static List<Contenido> GetArticleUser(int id)
        {
        ...
        public static Contenido GetOneArticle(int id)
        {
        ...
        {
            string nada = "";
            string cmd = GetGeneric(nada);
            return CrearLista(cmd);
        }
        public static List<Contenido> GetByPop()
        {
            string cmd = GetGeneric("ORDER BY contenido.IPopularidad DESC");
            return CrearLista(cmd);
        }
        public static List<Contenido> GetByDes()
        {
            string cmd = GetGeneric("ORDER BY contenido.IDescargas DESC");
            return CrearLista(cmd);
        }
        public static List<Contenido> Search(List<string> keywords)
        {
        ...
        protected static Contenido ReadArticle(MySqlDataReader reader)
        {
            Contenido article = new Contenido();
        ...
        protected static List<Contenido> CrearLista(string Comando)
        {
            List<Contenido> articles = new List<Contenido>();
            Comando = Paginar(Comando,Pagina);
            MySqlCommand cmd = DataBaseAccess.GenerateSqlCommand(Comando);
            using (cmd.Connection)
            {
                MySqlDataReader reader = cmd.ExecuteReader();
        ...
        protected static string Paginar(string Comando, int Pagina)
        {
            Pagina = ((Pagina - 1) * 12) + 1;
            Comando += " LIMIT " + Pagina + ",12";
            return Comando;
        }
